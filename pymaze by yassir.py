...
class Bonus(pygame.sprite.Sprite):
    def __init__(self, x, y):
        super().__init__()
        self.image = pygame.Surface((TILE_SIZE - 12, TILE_SIZE - 12))
        self.image.fill(YELLOW)
        self.rect = self.image.get_rect()
        self.rect.x = x * TILE_SIZE + 6
        self.rect.y = y * TILE_SIZE + 6
...
for row_index, row in enumerate(level_map):
    for col_index, char in enumerate(row):
        if char == "W":
            ...
            bonus = Bonus(col_index, row_index)
...
while running:
    ...
    bonus_text = font.render(f"Bonus: {bonus_count}", True, YELLOW)
    screen.blit(bonus_text, (10, 10))
